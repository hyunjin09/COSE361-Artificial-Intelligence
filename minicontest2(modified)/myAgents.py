...
class MyAgent(CaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    #find max score possible given number of food in a map
    self.totalFood = max(1, len(self.getFood(gameState).asList()) - 2)

    # find deadends using dead end filling algorithm
    check = []
    dead = []
  ...
  def updateWeight(self, gameState, nextState, action, reward):
    ...
    for feature in features:
      self.weights[feature] = weights[feature] + self.alpha * diff * features[feature]  # update the weights with alpha value 0.1
...
class OffensiveQAgent(MyAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    capsules = self.getCapsules(gameState)
    food = self.getFood(gameState).asList()
    if len(food) > 0: # first set the target as food
      self.curGoal = self.returnClosestFood(gameState)
    if len(capsules) > 0: # if capsules exist, set the target as capsule
  ...
  def chooseAction(self, gameState):
    ...
    # if not util.flipCoin(epsilon):  # explore with epsilon value
    #   return returnAction
    # else:
  ...
  def searchNewGoal(self, gameState, action):
    ...
    if len(ghosts) > 0:
      ...
      if ghostDist >= 6 or self.capsuleActivated(successor):
        self.maxFoodCarry = self.totalFood  # if the pacman is in no danger, attempt to eat all food
      else:
    ...
    if foodLeft <=2 or time - self.getMazeDistance(self.returnClosestOpening(successor), nextStatePos) <= 2.0 \
      or (len(ghosts) > 0 and gameState.getAgentState(self.index).numCarrying >= self.maxFoodCarry and not self.capsuleActivated(gameState) \
        and ghostDist < 6):
      self.curGoal = self.returnClosestOpening(successor)   # return home if all food is eaten | no time | eaten enough food when winning
    if capsuleCount > 0:  # prioritize searching for capsule
  ...
  def returnActionFromQ(self, gameState, actions):
    ...
    return random.choice(actionsChoice) # randomly select action with highest value
  ...
  def getFeatures(self, gameState, action): 
   ...
   # return features based on action
    ...
    # do not enter the dead end if agent is being chased and capsule is not activated
    temp = 0.0
    if gameState.getAgentState(self.index).isPacman:  
      if nextStatePos in self.deadEnds:
        if minDist < 6 and not self.capsuleActivated(gameState):
          temp = 1.0
    ...
    self.searchNewGoal(gameState, action) # search for new goal
  ...
  def observationFunction(self, gameState):
    # called before chooseAction function: for rewards and updating weights
...
class DefensiveQAgent(MyAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    # instead of patrolling all entrances, only patrol the center ones
  ...
  def returnBestActions(self, gameState):
    ...
    # select action that brings closest to the target/patrol point
    for action in actions:
      nextState = gameState.generateSuccessor(self.index, action)
  ...
  def searchNewGoal(self, gameState):
    ...
    if self.curGoal == None:  # check whether a food has been eaten by an unspotted enemy
      ...
      if len(checkFood) > 0:
        self.curGoal = checkFood.pop() # if a food has been eaten, go toward the eaten food
    self.prevFood = self.getFoodYouAreDefending(gameState).asList()
